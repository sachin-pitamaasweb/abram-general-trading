...
export default function ProductShowcase({ mainImage, additionalImages, className, bgGradient = "linear-gradient(90deg, #FFFFFF -5%, #2384F6 100%, #52A2FF 45%)" }) {
    const [isHovered, setIsHovered] = useState(false);

    const features = [
        {
            icon: <Award className="h-6 w-6 sm:w-8 sm:h-8 text-gray-700" />,
            title: "Authentic Basmati",
        },
        {
    ...
    return (
        ...
        >
            ...
            >
                ...
                {additionalImages.slice(0, 2).map((src, index) => (
                    ...
                    >
                        <Image
                            ...
                            width={140}
                            height={200}
                            className="w-[80px] sm:w-[100px] md:w-[140px] h-auto object-contain"
                        />
                    </motion.div>
                ))}
                ...
                {additionalImages.slice(2).map((src, index) => (
                    <motion.div
                        key={index + 2}
                        initial={{ opacity: 0, scale: 0.9 }}
                        animate={{ opacity: isHovered ? 1 : 0, scale: isHovered ? 1.05 : 0.9 }}
                    ...
                    >
                        <Image
                            src={src || "/placeholder.svg"}
                            alt={`Additional Product ${index + 3}`}
                            width={140}
                            height={200}
                            className="w-[80px] sm:w-[100px] md:w-[140px] h-auto object-contain"
                        />
            ...
            >
                <h2 className="text-white text-2xl sm:text-3xl md:text-3xl font-bold">MAHRA</h2>
            </motion.div>
